...
# backend, so the request carries a routable Host header. A privileged endpoint
# port moves to the in-sandbox port and the authority carries that port with
# it, which is correct: Host routers match the hostname and ignore the port.
# Conversely, an IPv4 endpoint must not gain a hosts-alias flag.
cat > "$bin/curl" <<'CURL'
#!/usr/bin/env bash
printf '%s\n' '{"data":[{"id":"test-model"}]}'
CURL
...
if grep -qx -- '--hosts-alias' "$work/hostname endpoint.args.text" && \
    ...
    no "hostname endpoint keeps its hostname authority"
fi

# An image toolchain leaves FS_PI_NODE empty because pi runs in the image. A
