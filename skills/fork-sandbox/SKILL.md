...
3. **Launch it.**
   ...
   **It costs sessions.** Every iteration is up to two more sessions at the
   ...
   two. On a sealed run with no `--review-harness` (or a sealed one given
   ...
   into N paid sessions on whichever model it names, even though the
   implement leg stays sealed.
   ...
   **It names its own model, and only its own.** Unlike `--review-model`,
   ...
   `claude`/`pi`/`codex` choices as `--review-harness`; without
   it the legs run on the implement harness, with `--maintainer-model`
   substituting for the implement model. A sealed implement leg with
   ...
   **Watching one is no different**, for the same reason a review loop
   ...
   `pi` and `codex` have no hook system to measure context with,
...
## The other harnesses: `pi` and `codex`
...
```bash
fork-sandbox run --harness pi --network sealed --branch "<branch>" "<path>" "<handoff>"
```
...
- **No session resume.** `--session-state` and `--resume-session` are refused. `--session-state` binds a host directory over the claude CLI's transcript store so the conversation outlives the run, and a pod has no host directory to bind — its filesystem dies with the Job — so there is nowhere for a later run to resume from; `--resume-session` needs that directory and is refused with it. A cluster wake is always a fresh session, with the handoff as its only continuity.
- **`--network sealed` and `codex` are refused.** `--harness pi` and `--harness claude` are the two agents this path runs — see "Model access" in `docs/kubernetes-runs.md` for why each needs the network shape it gets.
...
- **`--context-secret <name>` is `--k8s`-only**: the pod mounts that Kubernetes Secret read-only at `/work/context`, one file per key, for a credential the task needs. The Secret must carry the label `fork-sandbox/context=true` and must not carry `fork-sandbox/branch`. Names starting `fork-sandbox-` or ending `-claude-token` are refused. It cannot be combined with `--context-ro`, which uses the same path. The site creates and labels the Secret; see "Getting a Secret in" in `docs/kubernetes-runs.md`.
...
3. **Route the report through the artifact outbox, and say so in the handoff you write** — this is your job as the launching session, not something the person asking for the review should have to ask for. Tell the run to write its review to a file in the outbox (the preamble already names the directory and already says reports belong there), to repeat it as its final message, and **not to commit**.

The third point is where a review run goes wrong quietly, and the failure is not the one you would guess. A stray `REVIEW.md` left in the clone is **not** lost: the pod's entrypoint runs `git add -A` and commits whatever is uncommitted at run end, so the review comes home as a surprise commit on the branch — in a run whose whole premise was leaving the repository untouched. Naming the outbox is what keeps the report a report. It arrives at the `--outbox-dir` path the run prints (default `/var/tmp/claude-scratch/forks/k8s-<safe-branch>/outbox/`), and a review that committed nothing leaves the origin repo exactly as it was.
...
| `<run-dir>/summary.json` | the same facts structured — harness and its version, model, exit code, commits with subjects, `cost_usd`, `total_cost_usd` (the run plus any `--review-loop` or `--refresh-at` legs), `refresh` and `continuations` (`--refresh-at`'s own record, each entry also carrying `handoff_stale`), `usage` token counts, `author_email_unexpected` (addresses among the returned commits that don't match the repo's own identity and that a host-side rewrite could not fix — empty both when no mismatch ever occurred and when one did but `authorship_normalized` fixed it, so a consumer needs both fields to see every misattributed run), `authorship_normalized` (0, or the count of commits a mismatch made this run rewrite host-side to the repo's own identity — see `<run-dir>/authorship-normalize.log` and the summary.txt NOTICE for which addresses were replaced), and — on a run launched with `--session-state <dir>`, the claude-only flag that binds a host directory over the CLI's transcript store so the conversation outlives the run — `session_state` (that directory) and `session_id` (the newest transcript in it at run end, null if it stayed empty, and what a later run passes back as `--resume-session` to continue the same conversation). Those last two are absent, not null, on a run without the flag: their presence is how a caller tells a resumable run from one whose transcript died with the sandbox. For reading, not grepping |
| `<run-dir>/authorship-normalize.log` | present whenever a returned commit's author didn't match the repo's identity and a host-side rewrite was attempted, whatever the outcome — `fs_normalize_authorship`'s stderr, which is where the git error lands when the rewrite fails and the summary.txt WARNING points you |
| `<run-dir>/handoff.md` | the rendered prompt before Kubernetes YAML serialization; on `--k8s`, YAML clip chomping can normalize multiple terminal newlines before the pod receives it |
