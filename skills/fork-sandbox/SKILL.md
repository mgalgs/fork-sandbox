...
- **`--pi-args` and `--prompts-dir` are refused as not-yet-built**, not as permanently unsupported — each names a real capability the cluster path has not been wired up to carry yet.
